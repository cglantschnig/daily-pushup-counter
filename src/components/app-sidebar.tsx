...
import { ChevronRight, Dumbbell, History, Settings2 } from "lucide-react"
  ...
  return (
    <Sidebar variant="inset" collapsible="icon" className="border-none">
      ...
      <SidebarFooter className="p-3">
        <div className="rounded-2xl border border-sidebar-border/60 bg-sidebar-accent/35 p-2 group-data-[collapsible=icon]:hidden">
          <div className="flex items-center gap-2 rounded-xl px-2 py-2">
            <div className="rounded-full border border-sidebar-border/50 bg-sidebar p-1">
              <UserButton />
            </div>
          ...
          <Link
            ...
            className={cn(
              "mt-2 flex items-center justify-between rounded-xl border border-sidebar-border/70 bg-sidebar/70 px-3 py-2 text-sm font-medium text-sidebar-foreground/80 transition-colors hover:bg-sidebar-primary/12 hover:text-sidebar-foreground",
              isNavigationItemActive(pathname, settingsNavigationItem) &&
                "bg-sidebar-primary/12 text-sidebar-foreground"
            )}
          >
